...
class Sales extends Secure_Controller
{
	...
	public function complete()
	{
		...
		$data['total_profit']=$totals['total_profit'];
		...
		if($this->sale_lib->is_invoice_mode() || $data['invoice_number_enabled'] == TRUE)
		{
			...
			else
			{
				...
				$data['sale_id_num'] = $this->Sale->save($data['sale_status'], $data['cart'], $customer_id, $employee_id, $data['comments'], $invoice_number, $data["quote_number"], $data['payments'], $data['dinner_table'], $data['taxes'], $data['total_profit']);
		...
		elseif($this->sale_lib->is_quote_mode())
		{
			...
			else
			{
				...
				$data['sale_id_num'] = $this->Sale->save($data['sale_status'], $data['cart'], $customer_id, $employee_id, $data['comments'], $invoice_number, $quote_number, $data['payments'], $data['dinner_table'], $data['taxes'], $date['total_profit']);
		...
		else
		{
			...
			$data['sale_id_num'] = $this->Sale->save($data['sale_status'], $data['cart'], $customer_id, $employee_id, $data['comments'], NULL, NULL, $data['payments'], $data['dinner_table'], $data['taxes'], $data['total_profit']);
			//$data['total_points']=$this->$this->Customer->get_info($customer_id)->points;
			$data['sale_id'] = 'POS ' . $data['sale_id_num'];
	...
	private function _load_customer_data($customer_id, &$data, $stats = FALSE)
	{
		...
		if($customer_id != -1)
		{
			...
			if($package_id != NULL)
			{
				$package_name = $this->Customer_rewards->get_name($package_id);
				$points = $this->Customer->get_info($customer_id)->points;
				$points_percentage = $this->Customer_rewards->get_points_percent($package_id);
				...
				$data['customer_rewards']['percent']=$points_percentage;
			}
	...
	private function _reload($data = array())
	{
		...
		//$data['stock_locations']=$this->Sale->get_sale_location();
		//$data['stock_location'] = $this->sale_lib->get_sale_location();
		//$data['my_location']=2;
		$data['stock_location'] =$this->sale_lib->get_sale_location();
		$data['my_location']=$this->Sale->get_sale_location();
	...
	public function suspend()
	{
		...
		$data = array();
		$sales_taxes = array();
		if($this->Sale->save($sale_status, $cart, $customer_id, $employee_id, $comment, $invoice_number, $quote_number, $payments, $dinner_table, $sales_taxes,$data['total_profit']) == '-1')
		{
