...
class Receiving_lib
{
	...
	public function add_item($item_id, $quantity = 1, $item_location = NULL, $discount = 0, $price = NULL, $description = NULL, $serialnumber = NULL, $receiving_quantity = NULL, $include_deleted = FALSE)
	{
		...
		if($itemalreadyinsale)
		{
			...
			$items[$updatekey]['total_sale_price'] = $sale_price*$items[$updatekey]['quantity'];
		}
		else
		{
	...
	public function edit_item($line, $description, $serialnumber, $quantity, $discount, $price)
	{
		...
		if(isset($items[$line]))
		{
			...
			$line['total_sale_price'] = $line['sale_price']*$quantity;
			$this->set_cart($items);
		}
	...
	public function get_trans_total()
	{
		$total = 0;
		foreach($this->get_cart() as $item)
		{
			$total = bcadd($total, abs($this->get_item_total(($item['quantity']* $item['receiving_quantity']), $item['price'], $item['discount']))/2);
		}
