...
class Sale extends CI_Model
{
	...
	{
		$myid=-100;
		$query = $this->db->query("EXPLAIN `ospos_items`;");
		foreach($query->result_array() as $row)
		if ($row['Field']=='slid')
			$myid=$row['Default'];
		return $myid;
	}
	 public function get_sale_locations()
	{
		$myid=-100;
		$query = $this->db->query("EXPLAIN `ospos_items`;");
		foreach($query->result_array() as $row)
	 ...
	 */
	public function save(&$sale_status, &$items, $customer_id, $employee_id, $comment, $invoice_number, $quote_number, $payments, $dinner_table, &$sales_taxes, $total_profit, $sale_id = FALSE)
	{
		...
		$this->save_customer_rewards($customer_id, $sale_id, $total_amount, $total_amount_used, $total_profit);
	 ...
	 */
	private function save_customer_rewards($customer_id, $sale_id, $total_amount, $total_amount_used, $total_profit)
	{
		if (!empty($customer_id) && $this->config->item('customer_reward_enable') == TRUE)
		{
			...
			if (!empty($package_id))
			{
				...
				//$total_profit="80"; 
				$total_amount_earned = ($total_profit * $points_percent / 100);
