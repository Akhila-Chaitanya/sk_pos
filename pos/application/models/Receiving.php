...
class Receiving extends CI_Model
{
    ...
    public function remote_req($items)
	{
		...
		$req_data=array(
		'loc_id'=>$item['item_location'],
		'item_id'=>$item['item_id'],
		'count'=>$item['quantity']
		);
		if($item['item_location']!=$myid)
		$this->db->insert('xreq',$req_data);
		}
	...
	public function save($items, $supplier_id, $employee_id, $comment, $reference, $payment_type, $receiving_id = FALSE)
	{
		...
		foreach($items as $line=>$item)
		{
			...
			if($myid==$item['item_location'])
			{
			$item_quantity = $this->Item_quantity->get_item_quantity($item['item_id'], $item['item_location']);
			//$this->Item_quantity->save(array('quantity' => $myid, 'item_id' => $item['item_id'],
