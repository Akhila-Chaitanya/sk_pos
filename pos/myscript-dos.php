...
echo "\n";
$k=0;
$query_r="SELECT * FROM `ospos_sync_seq` WHERE `ser_id`='1' LIMIT 1";
...
while($output=$res_r->fetch_assoc())
{
	if($output['loc_id']==$myid)
	{
		...
		if ($time_now>$timestamp+(36*$distance)){
			$sync=1;
			$query_r="UPDATE `ospos_sync_seq` SET `act`='1'";
			$result=$con_r->query($query_r);
		}
...
if ($k!=0 && $k!=14 && $k!=2 && $k!=15 && $k!=26 && $k!=7 && $tables[$k]!='ospos_sync' && $tables[$k]!='ospos_sync_seq')
{
	echo ("\n for:     ".$tables[$k]."\n");
